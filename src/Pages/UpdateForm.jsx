...
export default function UpdateForm() {
  ...
  const xyz = localStorage.getItem('empEmail')
  ...
  function handleSubmit(e) {
    ...
    newData.append('skills', skills.join(','))
  ...
  useEffect(() => {
    axios.get(`/employee/find/${xyz}`)
      .then(res => {
        ...
        const formattedJoiningDate = new Date(eData.joiningdate).toISOString().split('T')[0];

        setEmpName(eData.empname)
        setEmpId(eData.empid)
        setAge(eData.age)
        setMarritalStatus(eData.marritalstatus)
        setCity(eData.city)
        setJoiningDate(formattedJoiningDate)
        setEmail(eData.email)
        setPassword(eData.password)
        // setPhoto(eData.image)
        setGender(eData.gender)
